...
const OptionSettingInfo = ({ isOpen, onToggle, showEmptyEtcBtn, onNavigateTab, projectnum, qnum, userPerm, lv3Options, responseCount }) => {
    ...
    const runInfoSave = async (type) => {
        console.log("[runInfoSave] START type =", type);
        console.log("[runInfoSave] 현재 activeJobRef =", activeJobRef.current);
        /*유효성 체크 */
        ...
        console.log("[runInfoSave] 초기화 완료: lineCount =", lineCountRef.current);
        activeJobRef.current = null;
        clearLog();
        await sleep(300);  
        ...
        try {
            ...
            if (!ok) {
                console.error("[runInfoSave] 분석 API 실패");
            ...
            if (job) {
                // 허브 조인 + 로딩 on
                console.log("[runInfoSave] 새 jobKey =", job);
                activeJobRef.current = job;
                setAnalyzing(true); // 먼저 ON
                const joined = await joinJob(job);
                console.log("[runInfoSave] joinJob() 결과 =", joined);
                ...
                try {
                    ...
                    console.error("[runInfoSave] 예외 발생", e);
                  } finally {
                    setSaving(false);
                  }
            } else {
