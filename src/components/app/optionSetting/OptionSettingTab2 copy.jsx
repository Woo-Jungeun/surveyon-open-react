    ...
    const GridRenderer = (props) => {
        ...
        */
        const onItemChange = useCallback((e) => {
            ...
            const nativeEvent = e.syntheticEvent;
          
            // --- 한글 조합 입력 중 처리 ---
            if (nativeEvent?.type === "compositionstart") {
              // 조합 시작 플래그만 세팅
              pendingTypingRef.current = true;
            }
            if (nativeEvent?.type === "compositionend") {
              // 조합 종료 → 200ms 뒤 커밋 예약
              pendingTypingRef.current = false;
              scheduleFlush();
            }
            ...
            // ⚠️ 여기서는 조합 중이어도 setDataState는 항상 허용 (focus 유지를 위해)
            setDataState(prev => {
              ...
              // ⚡️ 조합 중엔 커밋하지 않고, 끝나면 scheduleFlush가 자동 실행
              if (!pendingTypingRef.current) scheduleFlush();
            ...
            return (
                <td onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                    ...
                    <input
                        ...
                        onCompositionStart={() => (pendingTypingRef.current = true)}
                        onCompositionEnd={() => {
                            pendingTypingRef.current = false;
                            scheduleFlush();
                        }}
