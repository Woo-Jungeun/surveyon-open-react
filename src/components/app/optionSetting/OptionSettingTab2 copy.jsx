    ...
    */
    ...
    const GridRenderer = (props) => {
        ...
        const focusFirstErrorCell = useCallback(() => {
            console.log("focusFirstErrorCell");
        ...
        */
        ...
        const saveChanges = useCallback(async () => {
            ...
            if (!ok) {
                console.warn("[VALIDATION] ❌ 저장 중 필수값/중복 에러 감지됨");
                console.table(errors);
            
                setErrorMarks(new Map());
            
                // WeakMap은 객체 참조 깨지므로 key 기반 매칭
                setDataState(prevState => {
                    const nextRows = (prevState?.data || []).map((r, idx) => {
                        // WeakMap으로 바로 get하면 매칭 안됨 → key로 재탐색
                        const target = [...rowMarks.keys()].find(x =>
                            x.__rowKey === r.__rowKey || x.no === r.no
                        );
                        const set = target ? rowMarks.get(target) : null;
                        const kinds = target ? rowKinds.get(target) : null;
            
                        if (!set) {
                            if (r.__errors) console.log(`[ROW ${r.no}] ✅ 에러 해제됨`);
                            return { ...r, __errors: undefined, __errorKinds: undefined };
                        }
            
                        console.log(`[ROW ${r.no}] ❗에러필드:`, Array.from(set), kinds);
            
                        return {
                            ...r,
                            __errors: new Set(set),
                            __errorKinds: kinds,
                        };
                    });
            
                    console.log("[STATE] nextRows 중 에러 있는 행:",
                        nextRows.filter(r => r.__errors?.size > 0).map(r => ({
                            no: r.no,
                            errors: Array.from(r.__errors ?? []),
                            kinds: r.__errorKinds
                        }))
                    );
            
                    return { ...prevState, data: [...nextRows] }; // 새 배열로 강제 렌더
                });
                ...
                setTimeout(() => {
                    requestAnimationFrame(() => {
                      requestAnimationFrame(() => {
                        console.log("[FOCUS] 첫 에러 셀로 이동 시도 (2프레임 딜레이)");
                        focusFirstErrorCell();
                      });
                    });
                  }, 100);
