import { Calendar, DateInput, DatePicker, DateTimePicker } from "@progress/kendo-react-dateinputs";
 ...
 *
 */
const SearchFieldDatePickerRange = ({
    parentProps,
    required = false, // 필수여부
    label,
    format,
    ...
    const OPERATOR = operator || "between";
    ...
    useEffect(() => {
        if (defaultValue) {
            parentProps.defaultFilterChange({
                field: _fieldName,
                operator: OPERATOR,
                value: formatDateTime(defaultValue?.start, "start") + "," + formatDateTime(defaultValue?.end, "end")
            });
    ...
    const formatDateTime = (date, type) => {
        ...
        const min = dayjs(date).format("mm");
        const sec = dayjs(date).format("ss");
            ...
            if (max === null && isNaN(possibleRange)) {
                ...
                if (e.target.props.name === START_DATE_NAME && (_minData > _maxData || _possibleEndDate < _maxData)) {
                    parentProps.filterChange({
                        field: _fieldName,
                        operator: OPERATOR,
                        value: formatDateTime(_minData, "start") + "," + formatDateTime(_possibleEndDate, "end")
                    });
                    setMinData(_minData);
                    ...
                    parentProps.filterChange({
                        field: _fieldName,
                        operator: OPERATOR,
                        value: formatDateTime(_possibleStartDate, "start") + "," + formatDateTime(_maxData, "end")
                    });
    ...
    const possibleStartDate = (compareEndDate) => {
        ...
        if (isNaN(possibleRange)) {
            ...
            const _possibleDate = compareEndDate
                ? moment(compareEndDate).subtract(possibleRange, "minutes").toDate()
                : moment(maxData).subtract(possibleRange, "minutes").toDate();
            // "최대 일자에서 계산된 시작가능일자"가 "계산된 시작가능일자"보다 이 전일 경우? 최대 일자에서 계산된 시작가능일자: 계산된 시작가능일자
            return max !== null
                ? moment(max).subtract(possibleRange, "minutes").toDate() < _possibleDate
                    ? moment(max).subtract(possibleRange, "minutes").toDate()
                    : _possibleDate
                : _possibleDate;
        }
    };
    ...
    const possibleEndDate = (compareStartDate) => {
        ...
        if (isNaN(possibleRange)) {
            ...
            const _possibleDate = compareStartDate
                ? moment(compareStartDate).add(possibleRange, "minutes").toDate()
                : moment(minData).add(possibleRange, "minutes").toDate();
    ...
    const defaultEndDate = nowDayJs.toDate();
    const defaultStartDate = nowDayJs.subtract(1, "hour").toDate();
        ...
        <>
            ...
            <div className="inpArea cal">
                <DateTimePicker
                    ...
                    dateInput={(props) => {
                        return <DateInput {...props} readonly={true}></DateInput>;
                    }}
                    validationMessage={required ? message.searchFieldValidationMessage : "날짜시간이 올바르지 않습니다."}
                    required={required}
                    {...props}
                />
            </div>
...
SearchFieldDatePickerRange.propType = {
    parentProps: PropTypes.object.isRequired
    // name : PropTypes.object.isRequired,
};

export default SearchFieldDatePickerRange;
