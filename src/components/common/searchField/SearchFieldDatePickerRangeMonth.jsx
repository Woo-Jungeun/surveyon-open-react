 ...
 *
 */
const SearchFieldDatePickerRangeMonth = ({ parentProps,
                                        required = false, // 필수여부
                                        label,
                                        format,
                                        name,
    ...
    const possibleStartDate = (compareEndDate) => {
        ...
        if(isNaN(possibleRange)){
            ...
            const _possibleDate = compareEndDate ? moment(compareEndDate).subtract(possibleRange, 'month').toDate() : moment(maxData).subtract(possibleRange, 'month').toDate();
            // "최대 일자에서 계산된 시작가능일자"가 "계산된 시작가능일자"보다 이 전일 경우? 최대 일자에서 계산된 시작가능일자: 계산된 시작가능일자
            return max !== null ? (moment(max).subtract(possibleRange, 'month').toDate() < _possibleDate ? moment(max).subtract(possibleRange, 'month').toDate() : _possibleDate) : _possibleDate;
        }
    };
    // 가능한 종료일자
    const possibleEndDate = (compareStartDate) => {
        ...
        if(isNaN(possibleRange)){
            ...
            const _possibleDate = compareStartDate ? moment(compareStartDate).add(possibleRange, 'month').toDate() : moment(minData).add(possibleRange, 'month').toDate();
...
SearchFieldDatePickerRangeMonth.propType = {
    parentProps: PropTypes.object.isRequired,
    // name : PropTypes.object.isRequired,
}

export default SearchFieldDatePickerRangeMonth;