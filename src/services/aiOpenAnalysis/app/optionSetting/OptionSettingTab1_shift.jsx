    ...
    */
    ...
    const GridRenderer = memo(forwardRef((props, ref) => {
        ...
        const onRowClick = useCallback((e) => {
            ...
            // Shift 키가 눌렸고, 이전에 선택된 기준점(anchor)이 있다면 범위 선택
            if (e.nativeEvent.shiftKey && anchorIndexRef.current !== null && rowIndex >= 0) {
                ...
                // 단일 클릭 (Shift 없음) -> 선택 초기화하고 현재 행만 선택
            ...
            // inEdit 플래그: 클릭한 행만 true, 나머지는 false (단일 선택일 때만 편집 모드 진입이 자연스러움)
            // 다중 선택 시에도 마지막 클릭한 행은 편집 모드로 표시될 수 있음
            setDataState(prev => ({
                ...prev,
            ...
            return React.cloneElement(trEl, {
                ...
                onClick: (e) => {
                    ...
                    if (!isCheckbox && !isButton && !isSentiment) {
                        // Shift 키가 눌리지 않았을 때만 소분류 패널 자동 열기
                        if (!e.shiftKey) {
        ...
        return (
            <Fragment>
                ...
                <div ref={gridRootRef} id="grid_01" className={`cmn_grid singlehead ${isLeftOpen && String(lvCode) !== "1" ? "force-scroll" : ""} ${hasLv3CellSelection ? "lv3-cell-select" : ""} ${isDragging ? "is-dragging" : ""}`} style={{ marginBottom: '0px', userSelect: 'none' }}>
                    <KendoGrid
                        rowHeight={38}
                                                ...
                                                >
                                                    <div
                                                        ...
                                                        onClick={(e) => {
                                                            ...
                                                            // Shift 키 누른 상태에서 범위 선택
                                                            if (e.shiftKey && anchorIndexRef.current !== null) {
                                                                const start = Math.min(anchorIndexRef.current, cellProps.dataIndex);
                                                                const end = Math.max(anchorIndexRef.current, cellProps.dataIndex);
                                                                const nextSel = new Set();

                                                                const currentData = dataState?.data || [];
                                                                for (let i = start; i <= end; i++) {
                                                                    const r = currentData[i];
                                                                    if (r) nextSel.add(getKey(r));
                                                                }

                                                                setLv3SelKeys(nextSel);
                                                                lastIndexRef.current = cellProps.dataIndex;
                                                                // 범위 선택 시에는 패널 자동 열기 하지 않음 (사용자가 명시적으로 열도록 유도하거나, 마지막 클릭 기준 동작 방지)
                                                                return;
                                                            }

                                                            // Shift 키 없을 때: 기준점(anchor) 업데이트
                                                            anchorIndexRef.current = cellProps.dataIndex;
                                                            lastIndexRef.current = cellProps.dataIndex;

                                                            // 클릭한 행이 선택된 상태가 아니라면 하이라이트 적용
                                                            // 또는 Shift 키가 눌리지 않았을 때 단일 선택으로 리셋 (단, 이미 선택된 행이면 유지)
                                                            if (!lv3SelKeys.has(rowKey)) {
                                                                setLv3SelKeys(new Set([rowKey]));
                                                                setSelectedRowKey(rowKey);
                                                            }
                                                            // 이미 선택된 행을 클릭했다면 선택 유지 -> 다중 선택 패널 열기
