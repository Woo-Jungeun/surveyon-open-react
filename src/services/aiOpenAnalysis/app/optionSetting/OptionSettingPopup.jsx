  ...
  const GridRenderer = memo(forwardRef((props, ref) => {
    ...
    useEffect(() => {
      const end = () => {
        ...
        // 드래그가 실제로 일어났다면(인덱스가 다르면), 클릭 이벤트 차단
        if (anchorIndexRef.current !== lastIndexRef.current) {
          suppressNextClickRef.current = true;
        }
    ...
    const rowRender = (trEl, rowProps) => {
      ...
      const cls = `${base} ${clicked ? 'row-active' : ''} ${selectedByBatch ? 'lv3-row-selected' : ''} ${pending ? 'row-pending-delete' : ''}`.trim();
