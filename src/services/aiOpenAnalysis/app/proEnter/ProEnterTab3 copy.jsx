...
const ProEnterTab3 = () => {
    ...
    const handlePofFilterChange = async (event) => {
        ...
        if (filter && filter.length >= 4) { // 4글자 이상일 때 검색
            try {
                ...
                // API 응답이 배열이라고 가정 (예: [{ pofNumber: "...", pofName: "..." }, ...])
                // 만약 응답 구조가 다르면 수정 필요
                setPofList(Array.isArray(data) ? data : []);
            } catch (e) {
    ...
    const handlePofChange = (event) => {
        ...
        // 선택된 객체에서 조사명 자동 입력
        if (val && typeof val === 'object' && val.pofName) {
            setProjectname(val.pofName);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // --- 유효성 검사 ---
        // --- 유효성 검사 ---
        const pofValue = pof && typeof pof === 'object' ? pof.pofNumber : pof;
