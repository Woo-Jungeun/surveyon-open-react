...
const MenuBar = () => {
  ...
  // 매뉴얼 새창 열기
  const openManual = () => {
    const manualWindow = window.open('', 'manual', 'width=1280,height=900,scrollbars=yes');
    if (manualWindow) {
      const htmlContent = `
        <!DOCTYPE html>
        <html lang="ko">
        ...
        <body>
            ...
            <!-- 데이터 및 스크립트 로드 -->
            <script>
                const manualData = ${JSON.stringify(manualData)};
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    // 초기 로드 시 첫 번째 메뉴 선택
                    let currentId = manualData.length > 0 ? manualData[0].id : null;
                    ...
                    function renderMenu() {
                        ...
                        manualData.forEach(item => {
                            ...
                            button.className = \`menu-btn \${item.id === currentId ? 'active' : ''}\`;
                            button.innerHTML = \`\${item.icon}<span>\${item.title}</span>\`;
                            button.onclick = () => selectMenu(item.id);
                    ...
                    function renderContent(id) {
                        const item = manualData.find(data => data.id === id);
                        if (item) {
                            // 페이드 인 효과를 위해 opacity 조절
                    ...
                    function selectMenu(id) {
                        currentId = id;
                        renderMenu(); // 활성 상태 업데이트를 위해 다시 렌더링 (또는 클래스만 토글 가능)
                        renderContent(id);
                        
                        // 모바일에서 메뉴 클릭 시 스크롤 최상단으로 이동 및 메뉴 닫기
                        const mainContent = document.getElementById('main-content');
                        // mainContent가 overflow: auto가 아닐 수 있으므로 window 스크롤도 고려
                        window.scrollTo(0, 0);
      ...
      `;
      manualWindow.document.write(htmlContent);
