...
const buildSigPolicy = (sigTypeVal, excludeN, excludeEtc, level, diffMin, diffMax, excludeColUnderN, upColor, showArrow) => {
    ...
    return {
        sig_diff_fin_mode: mode,
        sig_diff_test_mode: mode === 'deviation',
        sig_level: Number(level ?? 95),
                                                    ...
                                                    {displayAiSummaryData.mode_a_demographic_analysis.map((item, idx) => (
                                                        <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '4px', fontSize: '13px', lineHeight: '1.6' }}>
                                                            ...
                                                            <div style={{ wordBreak: 'keep-all' }}>
                                                                ...
                                                                <span style={{ color: '#2563eb', fontWeight: 600, marginLeft: '6px', marginRight: '6px' }}>({item.max_min_gap})</span>
            ...
            if (!contextFetchedRef.current) {
                ...
                if (ctxPayload) {
                    ...
                    fetchedUi = {
                        ...
                        percent_symbol: ctxPayload.ui_settings?.percent_symbol ?? ctxPayload.ui_settings?.format_percent_symbol ?? ctxPayload.display_policy?.percent_symbol ?? true,
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (pageId && user) {
                                                const newUi = {
                                                    ...
                                                    sig_exclude_column_under_n: localSigExcludeColumnUnderN,
                                                    sig_up_color: localSigUpColor,
                                                    sig_show_arrow: localSigShowArrow,
                                                    percent_symbol: uiSettings?.percent_symbol ?? true,
                                                    percent_as_column: uiSettings?.percent_as_column ?? false,
                                                    stub_group_layout: uiSettings?.stub_group_layout ?? 'merge',
                                    ...
                                    try {
                                        ...
                                        const requestData = {
                                            ...
                                            display_policy: {
                                                show_n: showN,
                                                show_percent: showPct,
                                                percent_symbol: uiSettings?.percent_symbol ?? showPct,
                                                percent_as_column: uiSettings?.percent_as_column ?? false,
                                                stub_group_layout: uiSettings?.stub_group_layout ?? 'merge',
                                                hide_zero_base_columns: hideZeroBaseColumns,
                                                hide_zero_stubs: uiSettings?.hide_zero_stubs ?? false,
