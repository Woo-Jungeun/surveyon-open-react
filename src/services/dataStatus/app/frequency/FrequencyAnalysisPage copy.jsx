...
const FrequencyAnalysisPage = () => {
    ...
    const { getAggregationData } = FrequencyAnalysisPageApi();
            ...
            try {
                ...
                if (aggResult?.success === "777" && aggResult.resultjson) {
                    ...
                    setQuestions(prevQuestions => prevQuestions.map(q => {
                        ...
                        const doneCol = aggInfoCols.find(c => c.label === '완료');
                        const failCol = aggInfoCols.find(c => c.label === '선정탈락');
                        const quotaCol = aggInfoCols.find(c => c.label === '쿼터오버');
                        ...
                        const updatedData = q.data.map(opt => {
                            ...
                            if (row) {
                                done = doneCol && row[doneCol.key] !== undefined ? Number(row[doneCol.key]) : 0;
                                fail = failCol && row[failCol.key] !== undefined ? Number(row[failCol.key]) : 0;
                                quota = quotaCol && row[quotaCol.key] !== undefined ? Number(row[quotaCol.key]) : 0;

                                if (!doneCol && !failCol && !quotaCol && row.value !== undefined) {
                                    done = Number(row.value) || 0;
                                }
                            }
